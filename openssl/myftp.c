...
int _send(SSL* ssl, void* data, unsigned int data_len){
  int len, sd = SSL_get_fd(ssl);
  if((len=SSL_write(ssl, data, data_len)) < 0){
...
int _recv(SSL* ssl, void* data, unsigned int data_len){
  int len, sd = SSL_get_fd(ssl);
  if((len=SSL_read(ssl, data, data_len)) < 0){
...
void recv_data(SSL* ssl, void* data, unsigned int data_len){
  ...
  for(i=0; i<data_len;){
    len = _recv(ssl, data + i, i + CHUNK_SIZE < data_len ? CHUNK_SIZE : data_len - i);
    i += len;
